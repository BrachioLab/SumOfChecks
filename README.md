...
Each source has disjoint `few_shot` and `dev` image splits: Endoscapes 4/49, SAGES 6/54. SAGES retains all six v5 examples in manifest order, not a four-example experimental subsample. See [data/README.md](data/README.md) and the [executed example notebook](notebooks/load_hf_sum_of_checks.ipynb). Rubric labels are development annotations from a surgeon-trained ML PhD student, not expert ground truth.

Original video filenames/IDs and frame IDs for all ten selected exemplars are listed in the [few-shot reference table](data/README.md#few-shot-video-and-frame-reference).
